...
def depth_guided_sampling(model_mean: torch.Tensor, t: int, matches_dict: Dict, DGS_cfg: Dict):
    # pre-process matches
    ...
    # # # conduct DGS
    # model_mean = DGS_optimize(model_mean, t, processed_matches, **DGS_cfg) #[B, frame_num, pose_dim]

    # # Optimize FL, R, and T separately
    # model_mean = DGS_optimize(
    #     model_mean, t, processed_matches, update_T=False, update_R=False, update_FL=True, **DGS_cfg
    # )  # only optimize FL

    # model_mean = DGS_optimize(
    #     model_mean, t, processed_matches, update_T=False, update_R=True, update_FL=False, **DGS_cfg
    # )  # only optimize R

    # model_mean = DGS_optimize(
    #     model_mean, t, processed_matches, update_T=True, update_R=False, update_FL=False, **DGS_cfg
    # )  # only optimize T

    model_mean = DGS_optimize(model_mean, t, processed_matches, **DGS_cfg)
...
def DGS_optimize(
    ...
    update_T: bool = True,
    update_FL: bool = True,
    # the args below come from **DGS_cfg
    alpha: float = 0.0001,
